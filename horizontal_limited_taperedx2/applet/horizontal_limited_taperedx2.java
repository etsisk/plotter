import processing.core.*; 
import processing.xml.*; 

import java.applet.*; 
import java.awt.Dimension; 
import java.awt.Frame; 
import java.awt.event.MouseEvent; 
import java.awt.event.KeyEvent; 
import java.awt.event.FocusEvent; 
import java.awt.Image; 
import java.io.*; 
import java.net.*; 
import java.text.*; 
import java.util.*; 
import java.util.zip.*; 
import java.util.regex.*; 

public class horizontal_limited_taperedx2 extends PApplet {

ArrayList pts;
int thresh;

public void setup()
{
  size(4000, 2000);
  frameRate(30);
  background(0xffffffff);
  thresh = 200;
  pts = new ArrayList();
  noStroke();
  smooth();
}

public void draw()
{
  int currentTime = millis();
  float x = random(width);
  float y = random(1000 - currentTime/600, 1000 + currentTime/600);
  float edgeX = -1.0f;
  if(x < 40.0f) edgeX = x;
  if(x - width > -40.0f) edgeX = abs(x - width);
  if(edgeX != -1.0f) y = random(1000 - edgeX * 10, 1000 + edgeX * 10);
  float z = 0;
  float r = random(1, 8);
  if(pts.size() < 2011)
  {
    DataPoint p = new DataPoint(x, y, r);
    pts.add(p);
    p.recordPoint(p);
    println(currentTime);
  }
  else{ println("END IT!");}
}

class DataPoint
{
  float x, y, r, xd, yd, td;
  int connections;
  DataPoint (float xPos, float yPos, float rad)
  {
    connections = 0;
    x = xPos;
    y = yPos;
    r = rad;
    fill(0, 0, 0, 50);
    ellipse(x, y, r, r);
  }
  public void recordPoint(DataPoint p)
  {
    for(int i = pts.size() - 1; i >= 0; i--)
    {
      DataPoint storedPoint = (DataPoint) pts.get(i);
      xd = abs(storedPoint.x - p.x);
      yd = abs(storedPoint.y - p.y);
      xd *= xd;
      yd *= yd;
      td = sqrt(xd + yd);
      if(td < thresh)
      {
        stroke(0, 0, 0, td/thresh * 100);
        strokeWeight(1 / td/thresh);
        if(storedPoint.connections < 50)
        {
          line(p.x, p.y, storedPoint.x, storedPoint.y);
          addConnection(p);
          addConnection(storedPoint);
        }
      }
    }
    //println("point " + p + " has no of connections: " + connections);
  }
  public void addConnection(DataPoint p)
  {
    p.connections += 1;
    //p.redraw(p);
  }
  public void redraw(DataPoint p)
  {
    float rad = p.r * p.connections;
    p.r = rad;
    fill(0, 0, 0, 10);
    ellipse(p.x, p.y, p.r, p.r);
  }
}
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#FFFFFF", "horizontal_limited_taperedx2" });
  }
}
